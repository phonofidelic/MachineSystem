...
public class MachineApiClient(IHttpClientFactory clientFactory) : IMachineApiClient
{
    ...
    {
        return await client.GetFromJsonAsync<GetMachinesResult>("/api/machines") 
    ...
    {
        var response = await client.PatchAsync($"/api/machines/{command.MachineId}/start", null) 
            ?? throw new MachineNotFoundException();

        var result = await response.Content.ReadFromJsonAsync<MachineActionResult>() 
